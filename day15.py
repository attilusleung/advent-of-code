...
cmap = [[" "] * 50 for i in range(50)]
posx = 25
posy = 25

stepcode = {1: (1, 3, 4), 2: (2, 3,4), 3:(1, 2, 3), 4:(1, 2, 4)}
movex = {1: 0, 2: 0, 3:-1, 4: 1}
movey = {1: -1, 2: 1, 3:0, 4: 0}

queue = [[i] for i in range(1, 5)]
visited = set()
step = None
...
while queue:
    ...
    if final == 1 or final == 2:
        if (posx + movex[1], posy+movey[1]) not in visited:
            queue.append(com + [1])
        if (posx + movex[2], posy+movey[2]) not in visited:
            queue.append(com + [2])
        if (posx + movex[3], posy+movey[3]) not in visited:
            queue.append(com + [3])
        if (posx + movex[4], posy+movey[4]) not in visited:
            queue.append(com + [4])
        cmap[posy][posx] = "."
